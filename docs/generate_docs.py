...
def get_metadata_and_data_values(
                ...
                for col_group in sheet["data_columns"].values():
                    for definition in col_group.values():
                        if "merge_pointer" in definition:
                            context = sheet["prism_data_object_pointer"].rstrip("-")
                            if (
                                ...
                                context = "/".join(
                                    context.split("/")[
                                        : -int(definition["merge_pointer"][0])
                                    ]
                                )
                            data_values.add(context + definition["merge_pointer"])
                        if "process_as" in definition:
                            for process in definition["process_as"]:
                                context = sheet["prism_data_object_pointer"].rstrip("-")
                                if (
                                    process["merge_pointer"][0].isdigit()
                                    and process["merge_pointer"][0] != "0"
                                ):
                                    context = "/".join(
                                        context.split("/")[
                                            : -int(process["merge_pointer"][0])
                                        ]
                                    )
                                data_values.add(context + process["merge_pointer"])
...
class AssaySchema:
    ...
    def process_assays(self):
        ...
        assay_metadata_values, assay_data_values = get_metadata_and_data_values(
            self.assays.values()
        )

        self.assay_metadata = self.process_metadata(assay_metadata_values)
        self.assay_data = self.process_data(assay_data_values)
    ...
    def process_analyses(self):
        ...
        analysis_metadata_values, analysis_data_values = get_metadata_and_data_values(
            self.analyses.values()
        )

        self.analysis_metadata = self.process_metadata(analysis_metadata_values)
        self.analysis_data = self.process_data(analysis_data_values)
    ...
    def process_metadata(self, metadata_values: Set[str]) -> Tuple[dict, List[str]]:
        metadata = {
            k: v.copy()
        ...
        for k in metadata:
            if "type" not in metadata[k] and "url" in metadata[k]:
                schema_path = (
                    metadata[k]["url"]
                    .replace(".", "/")
                    .replace("/html", ".json")
                ...
                merge_pointer = (
                    metadata[k]["url"].split("#")[-1]
                    if metadata[k]["url"].count("#")
                    else ""
                )
                schema = _load_schema(SCHEMA_DIR, schema_path)
                if merge_pointer in schema.get("properties", {}):
                    metadata[k].update(schema["properties"][merge_pointer])
                elif merge_pointer in schema.get("definitions", {}):
                    metadata[k].update(schema["definitions"][merge_pointer])
            if metadata[k].get("type") == "array":
                if "description" not in metadata[k]:
                    metadata[k]["description"] = metadata[k]["items"].get(
                        "description", ""
                    )

                if "type" not in metadata[k]["items"] and "url" in metadata[k]["items"]:
                    schema_path = (
                        metadata[k]["items"]["url"]
                        .replace(".", "/")
                        .replace("/html", ".json")
                        .split("#")[0]
                    )
                    merge_pointer = (
                        metadata[k]["items"]["url"].split("#")[-1]
                        if metadata[k]["items"]["url"].count("#")
                        else ""
                    )
                    schema = _load_schema(SCHEMA_DIR, schema_path)
                    if merge_pointer in schema.get("properties", {}):
                        metadata[k]["items"].update(schema["properties"][merge_pointer])
                    elif merge_pointer in schema.get("definitions", {}):
                        metadata[k]["items"].update(
                            schema["definitions"][merge_pointer]
                        )
                    elif not merge_pointer:
                        pass

        return metadata

    def process_data(self, data_values: Set[str]):
        data = {}
        if len(data_values):
            for k in sorted(data_values):
                ...
                for level in levels:
                    ...
                    if level in root:
                        ...
                        while "items" in root or "properties" in root or "url" in root:
                            if "items" in root:
                                root = root["items"]
                                ...
                                root = _load_schema(SCHEMA_DIR, schema_path)
                                if merge_pointer in root.get("definitions", {}):
                                    # definitions first because properties can point here
                                    root = root["definitions"][merge_pointer]
                                elif merge_pointer in root.get("properties", {}):
                                    root = root["properties"][merge_pointer]
                                elif not merge_pointer:
                                    # already reset root
                                    pass
                                else:
                        ...
                        print(self.name, k, "skipping", level)
                        print("\tnot in", list(root.keys()))
                ...
                if k.endswith("-"):
                    if "url" in root:
                        ...
                        if "/" in schema_path and "artifacts" not in schema_path:
                            merge_pointer = (
                                root["url"].split("#")[-1]
                                if root["url"].count("#")
                                else ""
                            ...
                            root = _load_schema(SCHEMA_DIR, schema_path)
                            if merge_pointer in root.get("properties", {}):
                                root = root["properties"][merge_pointer]
                            elif merge_pointer in root.get("definitions", {}):
                                root = root["definitions"][merge_pointer]
                            elif not merge_pointer:
                                pass
