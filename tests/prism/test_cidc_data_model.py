...
def test_merge_patch_into_trial(prism_test: PrismTestData):
    ...
    # Ensure the merge result differs only by upload placeholder UUIDs
    diff = DeepDiff(result, prism_test.target_trial)
    if "values_changed" in diff:
        assert len(diff["values_changed"]) == len(prism_test.upload_entries)
    else:
        # For manifest uploads, we expect no artifacts, so there should
...
def test_merge_artifacts(prism_test: PrismTestData):
    ...
    for uuid, artifact in uuids_and_artifacts:
        ...
        path = paths.pop().replace("['upload_placeholder']", "")
