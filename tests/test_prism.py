...
def test_end_to_end_prismify_merge_artifact_merge(xlsx, template):
    ...
    if template.type == "ihc":
        ...
        # 14 (for each run) + 15 (for each tumor sample) + 15 (for each normal sample) + 2 (for neoantigen)
        assert len(merged_gs_keys) == 2 * (14 + (15 * 2) + 2)
    ...
    if template.type == "wes_fastq":
        ...
        # 7 artifact attributes * 2*(14+(15*2)+2) files
